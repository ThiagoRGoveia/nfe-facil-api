...
# Script to generate the HttpClientLib and move original source files.

# Exit on any error
set -e

LIB_NAME="HttpClientLib"
LIB_PREFIX="defaultLibraryPrefix"

SOURCE_FILES_TO_MIGRATE=(
  "apps/api/src/infra/http/http.service.adapter.ts"
  "apps/api/src/core/ports/http.service.port.ts"
)
...
echo "   - Service: ${TARGET_LIB_SRC_DIR}/${LIB_NAME_KEBAB}.service.ts (can be removed or adapted)"
...
echo "   - http.service.adapter.ts is in '${TARGET_LIB_SRC_DIR}/${MIGRATED_SOURCE_SUBDIR}/apps/api/src/infra/http/'"
echo "   - http.service.port.ts is in '${TARGET_LIB_SRC_DIR}/${MIGRATED_SOURCE_SUBDIR}/apps/api/src/core/ports/'"
echo "   - Copy 'http.service.port.ts' into '${TARGET_LIB_SRC_DIR}/http.service.port.ts'."
echo "   - Copy 'http.service.adapter.ts' into '${TARGET_LIB_SRC_DIR}/http.service.adapter.ts'."
echo "     This adapter should implement 'HttpServicePort'."
echo "   - Update '${LIB_NAME_KEBAB}.module.ts':"
echo "     - Import 'HttpModule' from '@nestjs/axios'. You can register it directly: 'HttpModule.register({})' or use 'HttpModule.registerAsync' if config is needed."
echo "     - Add 'HttpModule' to the 'imports' array of '${LIB_NAME_KEBAB}.module.ts'."
echo "     - Provide and export 'HttpServicePort' using 'HttpServiceAdapter':"
echo "       providers: [{ provide: HttpServicePort, useClass: HttpServiceAdapter }],"
echo "       exports: [HttpServicePort],"
echo "   - Ensure 'HttpServiceAdapter' correctly injects 'HttpService' from '@nestjs/axios'."
echo "   - The generated '${LIB_NAME_KEBAB}.service.ts' can likely be removed if 'HttpServiceAdapter' fulfills its role."
echo ""
echo "4. Ensure the main 'index.ts' exports the module, port, and adapter."
echo ""
