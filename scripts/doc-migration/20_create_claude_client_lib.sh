...
# Script to generate the ClaudeClientLib and move original source files.

# Exit on any error
set -e

LIB_NAME="ClaudeClientLib"
LIB_PREFIX="defaultLibraryPrefix"

SOURCE_FILES_TO_MIGRATE=(
  "apps/api/src/infra/ai/claude/claude.client.ts"
)
...
echo "   - Service: ${TARGET_LIB_SRC_DIR}/${LIB_NAME_KEBAB}.service.ts (can be adapted to be the ClaudeClient or removed)"
echo "   - Index:   ${TARGET_LIB_SRC_DIR}/index.ts"
echo ""
echo "3. Integrate logic from the original file (now in '${TARGET_LIB_SRC_DIR}/${MIGRATED_SOURCE_SUBDIR}/apps/api/src/infra/ai/claude/claude.client.ts'):"
echo "   - Copy the 'claude.client.ts' into '${TARGET_LIB_SRC_DIR}/claude.client.ts'."
echo "   - Update '${LIB_NAME_KEBAB}.module.ts' to provide and export the ClaudeClient:"
echo "     providers: [ClaudeClient],"
echo "     exports: [ClaudeClient],"
echo "   - Ensure any internal imports within the client (e.g., for Anthropic SDK, 'ConfigService') are updated and dependencies are managed."
echo "   - The generated '${LIB_NAME_KEBAB}.service.ts' can be removed if ClaudeClient is self-contained, or it can be adapted to be the ClaudeClient."
echo ""
echo "4. Ensure the main 'index.ts' exports the module and client."
echo ""
...
echo ""
echo "6. IMPORTANT: Update import paths in other parts of the monorepo that previously imported this client"
