...
# Script to generate the UsersLib and move original source files.
...
LIB_NAME="UsersLib"
LIB_PREFIX="lib"
SOURCE_MODULE_DIR="apps/api/src/core/users"
...
if [ ! -d "${SOURCE_MODULE_DIR}" ]; then
    ...
    if [ -z "$(ls -A ${SOURCE_MODULE_DIR})" ]; then
        ...
        # Note: The original directory ${SOURCE_MODULE_DIR} will be empty or removed if 'mv' moved the directory itself (if it was the only item).
        # It's safer to leave removal of the original empty directory as a manual step.
    fi
fi
...
echo "   - Examine '${MIGRATED_SOURCE_SUBDIR}/users.module.ts' (if it existed)."
...
echo "     They belong in the API-facing applications (e.g., 'api', 'public-api')."
...
echo "     Example: Logic from '${MIGRATED_SOURCE_SUBDIR}/users.service.ts' might go into '${LIB_NAME_KEBAB}.service.ts'."
echo ""
