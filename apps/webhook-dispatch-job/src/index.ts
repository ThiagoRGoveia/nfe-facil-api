...
async function bootstrap(): Promise<WebhookDispatchJobService> {
  logger.log('COLD START: Initializing NestJS Application');
  const app = await NestFactory.createApplicationContext(AppModule);
  app.useLogger(app.get(PinoLogger));
  const service = app.get(WebhookDispatchJobService);
  return service;
}

// Start bootstrapping immediately, don't wait for handler to be called
const serviceSubject = new ReplaySubject<WebhookDispatchJobService>();
bootstrap()
...
export const handler = async (event: SQSEvent) => {
  try {
    // Get the already bootstrapped service
...
if (process.env.NODE_ENV !== 'production' && process.env.NODE_ENV !== 'uat') {
  bootstrap()
    .then((service) => {
      service
        .processMessage({
          deliveryId: process.env.TEST_DELIVERY_ID || 'test-delivery-id',
        })
        .then(() => {
